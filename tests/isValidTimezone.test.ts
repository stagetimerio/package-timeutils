...
import { isValidTimezone } from '../src/index'
...
describe('isValidTimezone', () => {
  ...
  it('should throw an error if time zones are not available in the environment', () => {
    ...
    ;(global as unknown as { Intl: undefined }).Intl = undefined

    try {
      expect(() => isValidTimezone('America/New_York')).to.throw(
        'Time zones are not available in this environment'
      )
    } finally {
      // Restore the original Intl object
      ;(global as unknown as { Intl: typeof Intl }).Intl = originalIntl
    }
  })
})
