 ...
 *
 * ### DST
 * Zones that *transition exactly at midnight* (extremely rare, e.g. historical
 * Brazil) resolve to the next valid local instant — whatever `getToday` produces.
 *
...
export function parseCalendarDay (
  day: string | null,
  { timezone = 'UTC', now }: ParseCalendarDayOptions = {},
): Date {
  if (day == null) return getToday(timezone, now)
